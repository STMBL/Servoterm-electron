...
function keypress(e) {
   ...
   if(e.keyCode == 13 && connected){//enter
      ...
      //println(cmd.value);
      //println(connid);
      connid.write(convertStringToArrayBuffer(cmd.value + '\n'), (err) => {
...
function connected_cb(){
   println("connected");
   connid.resume()
   connid.on('data', receive);
   connid.on('error', error);
   connid.on('disconnect', error);
   connid.resume()
   connected = true;
   //connid.on('data',receive)
...
function getdevs(err, devices){
   for (var i = 0; i < devices.length; i++) {
      if((devices[i].vendorId && devices[i].vendorId == 1155 && devices[i].productId && devices[i].productId == 22336)){
			path = devices[i].comName;
         println("Connecting to " + path);
         //chrome.serial.connect(devices[i].path, connected_cb);
         connid = new sp(path, connected_cb)
         return;
      }
